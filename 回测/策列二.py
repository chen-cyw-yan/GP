...
def has_limit_up(df, window=15, exclude_last=5, limit_pct=9.8):
    """
    判断：从（当前往前 window 天）到（当前前 exclude_last 天）之间是否有涨停
    
    参数：
    window: 总回溯天数（默认15天）
    exclude_last: 排除最近几天（默认5天）
    """
    pct = df["close"].pct_change() * 100
    
    # 取区间：[-window : -exclude_last]
    target = pct.iloc[-window:-exclude_last]
    
    return target.max() >= limit_pct
...
def volume_expand(df, base_days=5, min_ratio=1.5, max_ratio=3):# 成交量放大，1.5-2倍
    vol = df["volume"]
...
def filer_stock():
    ...
    df = pd.read_sql(f"select * from gp.stock", con=conn)
    base_df = df
    result = []
    ...
    logger.info('剔除高位票')
    # df=df.loc[~(df['异动类型'].str.contains('10日涨跌幅')|df['异动类型'].str.contains('30日涨跌幅'))]
    # df=df.loc[df['异动类型'].str.contains('日涨跌幅')]
    logger.info('剔除高位票完成')
    ...
    res_df=res_df.loc[~(res_df['是否异动类型'].str.contains('10日涨跌幅')|res_df['是否异动类型'].str.contains('30日涨跌幅'))]
...
if __name__ == '__main__':
    ...
    res=res_df[['代码','日期']]
    res=res.rename(columns={'代码':'code','日期':'date'})

    engine = bk.BacktestEngine(res,sell_config=sell_config)
    engine.run()

    pd.DataFrame(engine.trade_log).to_excel(r"C:\Users\cyw\Desktop\jupyternotebook\git-python\GP\回测\trades.xlsx", index=False)
    engine.equity_df.to_excel(r"C:\Users\cyw\Desktop\jupyternotebook\git-python\GP\回测\equity.xlsx", index=False)