...
def main():
    ...
    # df_to_fs_wiki=df_save.loc[df_save['stock_code'].astype(str)==last_day]
    df_to_fs_wiki=df_tmp.loc[df_tmp['trade_date'].astype(str)==last_day]
    df_to_fs_wiki=df_to_fs_wiki[[
                        "stock_code",
                        "stock_name",
                        "trade_date",
                        "close_price",
                        "trigger_count",
                        "is_abnormal_type",
                        "next_day_may_trigger",
                        "min_required_change",
                        "target_level",
                        "warning_info"]]
    df_to_fs_wiki=df_to_fs_wiki.rename(columns={
    "stock_code":"代码",
    "stock_name":"名称" ,
    "trade_date":"日期" ,
    "close_price":"收盘价" ,
    "trigger_count":"触发信号次数",
    "is_abnormal_type":"是否异动类型" ,
    "next_day_may_trigger":"下一日可能触发" ,
    "min_required_change":"所需最小涨幅" ,
    "target_level":"目标等级",
    "warning_info":"预警信息"})
    print(df_to_fs_wiki)
    for index, row in df_to_fs_wiki.iterrows():
        ...
        dt = {
            ...
            "是否异动类型": str(row['是否异动类型']) if pd.notna(row['是否异动类型']) else "",
            "下一日可能触发": str(row['下一日可能触发']) if pd.notna(row['下一日可能触发']) else "",
            "目标等级": str(row['目标等级']) if pd.notna(row['目标等级']) else "",
            "预警信息": str(row['预警信息']) if pd.notna(row['预警信息']) else "",
            
            # --- 转为数字 (int/float) ---
            # 使用 pd.to_numeric 可以安全地将字符串或数字转为数值类型，errors='coerce' 会将无法转换的变为 NaN
            "收盘价": pd.to_numeric(row['收盘价'], errors='coerce'),
            "触发信号次数": pd.to_numeric(row['触发信号次数'], errors='coerce'),
            "所需最小涨幅": pd.to_numeric(row['所需最小涨幅'], errors='coerce'),
        ...
        dt["触发信号次数"] = 0 if pd.isna(dt["触发信号次数"]) else dt["触发信号次数"]
        dt["所需最小涨幅"] = 0 if pd.isna(dt["所需最小涨幅"]) else dt["所需最小涨幅"]
        rs=fs_client.insert_table_record(app_token='VFjWwh7EAifRtJkSjqvcnjtrnkf',table_id='tblgLgmECsrqfI16',line=dt)
